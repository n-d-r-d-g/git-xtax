...
from git_xtax.utils import bold, compact_dict, debug, popen_cmd, warn
...
class GitHubToken(NamedTuple):
    ...
    def __get_token_from_gh(cls, domain: str) -> Optional["GitHubToken"]:
        ...
        if gh_version_match:
            ...
            raise UnexpectedXtaxException(f"Could not parse output of `gh --version`: `{gh_version_stdout}`")
    ...
    def __fire_github_api_request(self, method: str, path: str, request_body: Optional[Dict[str, Any]] = None) -> Any:  # noqa: KW
        ...
        try:
            ...
            if err.code == http.HTTPStatus.UNPROCESSABLE_ENTITY:
                ...
                last_line = 'You can also use a different token provider - see `git xtax help github` for details.'
                if self.__token:
                    raise XtaxException(
            ...
            # unlike 307 for GET, or 301/302 for all HTTP methods.
                ...
                raise XtaxException(f'GitHub API returned `{err.code}` '
                                       f'HTTP status with error message: `{err.reason}`.')  # pragma: no cover
            else:
                raise UnexpectedXtaxException(f'GitHub API returned `{err.code}` HTTP status with error message: `{err.reason}`.')
...
GITHUB_CLIENT_SPEC = CodeHostingSpec(
    ...
    git_config_keys=CodeHostingGitConfigKeys(
        domain='xtax.github.domain',
        organization='xtax.github.organization',
        repository='xtax.github.repository',
        remote='xtax.github.remote',
        annotate_with_urls='xtax.github.annotateWithUrls',
        force_description_from_commit_message='xtax.github.forceDescriptionFromCommitMessage',
        pr_description_intro_style='xtax.github.prDescriptionIntroStyle',
    )
)
