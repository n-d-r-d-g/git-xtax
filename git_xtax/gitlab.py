...
from git_xtax.code_hosting import (CodeHostingClient,
                                      CodeHostingGitConfigKeys,
                                      CodeHostingSpec,
                                      OrganizationAndRepository,
                                      OrganizationAndRepositoryAndGitUrl,
                                      PullRequest)
from git_xtax.exceptions import XtaxException, UnexpectedXtaxException
from git_xtax.git_operations import LocalBranchShortName
from git_xtax.utils import compact_dict, debug, map_truthy_only, popen_cmd
        ...
        try:
            ...
            if err.code == http.HTTPStatus.CONFLICT:
                ...
                if 'Another open merge request already exists for this source branch:' in error_reason:
                    raise XtaxException(error_reason)
                else:
                ...
                last_line = 'You can also use a different token provider - see `git xtax help gitlab` for details.'
                if self.__token:
                    raise XtaxException(
                ...
                if error_response.get("message") == "Non GET methods are not allowed for moved projects":
                    raise XtaxException(
                        ...
                        "Please report this error as a comment under `https://github.com/VirtusLab/git-xtax/issues/1212`.\n"
                    ...
                    UnexpectedXtaxException(
                        f'GitLab API returned 405 (Method Not Allowed) HTTP status with error message: `{err.reason}`.')
            elif err.code >= 500:
                raise XtaxException(f'GitLab API returned `{err.code}` '
                                       f'HTTP status with error message: `{err.reason}`.')  # pragma: no cover
            else:
                raise UnexpectedXtaxException(f'GitLab API returned `{err.code}` HTTP status with error message: `{err.reason}`.')
        except OSError as e:  # pragma: no cover
            raise XtaxException(f'Could not connect to {url_prefix}: {e}')
...
GITLAB_CLIENT_SPEC = CodeHostingSpec(
    ...
    git_config_keys=CodeHostingGitConfigKeys(
        domain='xtax.gitlab.domain',
        organization='xtax.gitlab.namespace',
        repository='xtax.gitlab.project',
        remote='xtax.gitlab.remote',
        annotate_with_urls='xtax.gitlab.annotateWithUrls',
        force_description_from_commit_message='xtax.gitlab.forceDescriptionFromCommitMessage',
        pr_description_intro_style='xtax.gitlab.mrDescriptionIntroStyle',
    )
)
