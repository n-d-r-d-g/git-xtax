...
from .exceptions import UnderlyingGitException, UnexpectedXtaxException
...
class AnyRevision(str):
    ...
    def of(value: str) -> "AnyRevision":
        if not value:
            raise UnexpectedXtaxException(f'AnyRevision.of should not accept {value} as a param.')
...
class AnyBranchName(AnyRevision):
    ...
    def of(value: str) -> "AnyBranchName":
        if not value:
            raise UnexpectedXtaxException(f'AnyBranchName.of should not accept {value} as a param.')
...
class GitContext:
    ...
    def get_git_version(self) -> Tuple[int, int, int]:
        if not self.__git_version:
            ...
            if not raw:
                raise UnexpectedXtaxException(f"Could not parse output of `git version`: `{version_stdout}`")
        ...
        if not self.__main_worktree_root_dir:
            if self.get_git_version() < (2, 5):
                ...
                if first_entry.startswith('worktree '):
                    ...
                    raise UnexpectedXtaxException("Could not obtain the main worktree path from "
    ...
    def get_git_timespec_parsed_to_unix_timestamp(self, date: str) -> int:
        ...
        # Apparently `git rev-parse --since` always prints out a result, even for gibberish inputs
            ...
            raise UnexpectedXtaxException(f"Cannot parse timespec: `{date}`")
        ...
        if self.__config_cached is None:
            ...
            for config_entry in filter(None, git_config_stdout.split("\0")):
                ...
                if len(key_and_value_lines) == 2:
                    ...
                    raise UnexpectedXtaxException(f"Cannot parse config entry: {config_entry}.")
    ...
    def do_remote_branches_exist(self, remote: str, *branches: LocalBranchShortName) -> Dict[LocalBranchShortName, bool]:  # noqa: KW
        ...
        for line in result.stdout.splitlines():
            if '\t' not in line:
                raise UnexpectedXtaxException(f"Malformed line in git ls-remote output: {repr(line)}")
            ref = line.split('\t')[1]
            if not ref.startswith('refs/heads/'):
                raise UnexpectedXtaxException(f"Unexpected ref format in git ls-remote output: {repr(line)}")
    ...
    def rebase_onto(self, onto: AnyRevision, upstream: AnyRevision, branch: LocalBranchShortName) -> None:
        """Deterministic rebase: git rebase --onto <onto> <upstream> <branch>.

        Used by sync to rebase each branch onto its parent, using the remote
        tracking branch as upstream. No fork-point inference.
        """
        opts = ["--empty=drop"] if self.get_git_version() >= (2, 26, 0) else []
        self._run_git("rebase", *opts, "--onto", onto, upstream, branch, flush_caches=True)
    ...
    def rebase(self, onto: AnyRevision, from_exclusive: AnyRevision, branch: LocalBranchShortName,  # noqa: KW101
        ...
        try:
            ...
            # In our case, this can happen when git version invoked by git-xtax to start the rebase
            # is different than git version used (outside of git-xtax) to continue the rebase.
            # This used to be the case when git-xtax was installed via a strict-confinement snap
            ...
            # (snapped git-xtax uses whatever git is available in the host system),
            ...
            if os.path.isfile(author_script):
                ...
                # See https://github.com/VirtusLab/git-xtax/issues/935 for why author-script needs to be saved in this manner
