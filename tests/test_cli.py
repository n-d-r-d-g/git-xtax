...
class TestView:
    def test_view_shows_stack(self, git_repo):
        ...
        run_xtax("append", "feature-b", cwd=git_repo)
...
class TestAppend:
    def test_append_branch_onto_managed(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        run_git("checkout", "feature-a", cwd=git_repo)
        result = run_xtax("append", "feature-b", cwd=git_repo)
        assert result.returncode == 0
        assert "Appended" in result.stdout

    def test_append_with_onto_flag(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        result = run_xtax("append", "feature-b", "--onto=feature-a", cwd=git_repo)
        assert result.returncode == 0
        assert "Appended" in result.stdout
        assert "stack s" in result.stdout

    def test_append_duplicate_fails(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        result = run_xtax("append", "feature-a", "--onto=feature-a", cwd=git_repo)
        assert result.returncode != 0
        assert "already in stack" in result.stderr

    def test_append_onto_root_fails(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        result = run_xtax("append", "feature-b", "--onto=develop", cwd=git_repo)
        assert result.returncode != 0
        assert "not in any stack" in result.stderr

    def test_append_on_root_branch_fails(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        # Currently on develop (root)
        result = run_xtax("append", "feature-b", cwd=git_repo)
        assert result.returncode != 0

    def test_append_resolves_stack_from_parent(self, git_repo):
        """add with --onto should find the right stack automatically."""
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        # No need to switch active stack — it resolves from --onto
        result = run_xtax("append", "feature-b", "--onto=feature-a", cwd=git_repo)
        assert result.returncode == 0

    def test_append_onto_branch_not_in_stack_fails(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
    ...
    def test_append_cross_stack_branch_fails(self, git_repo):
        ...
        result = run_xtax("append", "feature-a", "--onto=feature-c", cwd=git_repo)
        assert result.returncode != 0
        assert "already in stack" in result.stderr

    def test_append_inserts_between_parent_and_child(self, git_repo):
        """Appending onto a branch that has a child inserts in between."""
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        run_git("checkout", "feature-a", cwd=git_repo)
        run_xtax("append", "feature-b", cwd=git_repo)
        # Now append feature-c onto feature-a (which already has child feature-b)
        run_git("checkout", "-b", "feature-c", "feature-a", cwd=git_repo)
        run_xtax("append", "feature-c", "--onto=feature-a", cwd=git_repo)
        # Stack should be: feature-a -> feature-c -> feature-b
        result = run_xtax("v", cwd=git_repo)
        output = result.stdout
        assert output.index("feature-a") < output.index("feature-c") < output.index("feature-b")

    def test_append_creates_nonexistent_branch_on_confirm(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        run_git("checkout", "feature-a", cwd=git_repo)
        result = run_xtax("append", "new-branch", cwd=git_repo, stdin="y\n")
        assert result.returncode == 0
        assert "Created branch" in result.stdout
        assert "Appended" in result.stdout
        # Verify branch was actually created
    ...
    def test_append_aborts_on_decline(self, git_repo):
        run_xtax("init", "s", "feature-a", "--root=develop", cwd=git_repo)
        run_git("checkout", "feature-a", cwd=git_repo)
        result = run_xtax("append", "new-branch", cwd=git_repo, stdin="n\n")
        assert result.returncode != 0
        assert "Aborted" in result.stderr
...
class TestNavigation:
    ...
    def test_integer_zero_goes_to_top(self, git_repo):
        self._setup_stack(git_repo)
        run_git("checkout", "feature-b", cwd=git_repo)
    ...
    def test_top_already_on_first(self, git_repo):
        self._setup_stack(git_repo)
        run_git("checkout", "feature-a", cwd=git_repo)
        result = run_xtax("top", cwd=git_repo)
        assert result.returncode == 0
    ...
    def test_bottom_already_on_last(self, git_repo):
        self._setup_stack(git_repo)
        run_git("checkout", "feature-b", cwd=git_repo)
        result = run_xtax("bottom", cwd=git_repo)
        assert result.returncode == 0
    ...
    def test_down_at_leaf_fails(self, git_repo):
        self._setup_stack(git_repo)
        run_git("checkout", "feature-b", cwd=git_repo)
        result = run_xtax("down", cwd=git_repo)
        assert result.returncode != 0
        assert "no children" in result.stderr
